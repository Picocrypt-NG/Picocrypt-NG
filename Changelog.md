...
<ul>
	...
	<li>✓ <strong>CLI input safety and migration</strong>: literal bracketed names stay literal, quoted explicit globs are available when intended, all positional operands are processed, and collisions between selected inputs or keyfiles and real output artifacts are refused without reserving obsolete temporary filenames. This is a deliberate CLI breaking change: scripts using the removed <code>-i</code>/<code>--input</code> flag must pass literal paths as positional operands instead; the old flag fails with migration guidance</li>
	<li>✓ <strong>Output safety</strong>: encryption and decryption reject destinations that alias an input, keyfile, or split chunk; volume writes use operation-owned random stages, while ZIP, split, recombine, and extraction use exclusive ownership and no-clobber publication so unrelated files are not truncated or deleted on failure</li>
	<li>✓ <strong>Android reliability</strong>: stale internal temporary files are removed before the UI is shown, a native operation is not started if its pre-start cleanup fails, failed force-decrypt retries do not bypass that cleanup, and cancelled operations no longer enter the successful save flow</li>
